...
attribute_goals(X) -->
        { collect_itf_goals(X, Gs),
          term_variables(Gs, Vs),
          maplist(call, Gs) },
        (   { dump(Vs, Vs, Constraints) } ->
            clist(Constraints),
            { itf_del(Vs) }
        ;   []
        ).

class:attribute_goals(X) --> itf:attribute_goals(X).

itf_del([]).
itf_del([V|Vs]) :- del_attr(V, itf), del_attr(V, class), itf_del(Vs).

collect_itf_goals(Term, Vs) :-
        term_variables(Term, Vs0),
        phrase(collect_itf_(Vs0), Vs).

collect_itf_([]) --> [].
collect_itf_([V|Vs]) -->
        (   { get_attr(V, class, VC) } ->
            { del_attr(V, class), collect_itf_goals(VC, VCs) },
            dlist(VCs),
            [put_attr(V, class, VC)]
        ;   []
        ),
        (   { get_attr(V, itf, VI) } ->
            { del_attr(V, itf), collect_itf_goals(VI, VIs) },
            dlist(VIs),
            [put_attr(V, itf, VI)]
        ;   []
        ),
